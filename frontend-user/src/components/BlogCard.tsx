...
const BlogCard = ({ title, excerpt, imageUrl, date, readTime, category, slug }: Props) => {
  return (
    <Link to={`/blog/${slug}`}>
        <article className="group bg-white dark:bg-zinc-800 rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border border-gray-200 dark:border-zinc-700 h-full">
        {/* Image Section */}
        ...
        <div className="p-6">
            ...
            {/* Read More Link */}
            <div className="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400 text-sm font-semibold hover:gap-3 transition-all duration-300">
            <span>Đọc thêm</span>
            <ArrowRight className="w-4 h-4" />
            </div>
        </div>
