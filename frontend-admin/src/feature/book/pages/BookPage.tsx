...
const CSS = `
  ...
  .bm-err { color:var(--red,#ef4444); font-size:10px; margin-top:4px; }
`
...
function BookModal({ book, categories, authors, publishers, onClose, onSave }: BookModalProps) {
    const [form, setForm] = useState<BookForm>(
        book
            ? {
                ...
                publisherId: "",
                images: book.images
                    ? [{ id: "existing-0", url: book.images, name: "cover", size: 0 }]
                    : [],
            }
            : EMPTY_FORM
    )
    ...
    const ErrMsg = ({ field }: { field: keyof BookForm }) =>
        errors[field] ? <p className="bm-err" style={{ ...mono }}>{errors[field]}</p> : null
...
/* ════════ DELETE CONFIRM ════════ */
function ConfirmDelete({ book, onClose, onConfirm, deleting }: {
    book: BookAdminResponse; onClose: () => void; onConfirm: () => Promise<void>; deleting: boolean
}) {
    return (
        <div
            className="bm-overlay"
...
export const BookManagementPage = () => {
    ...
    const handleSave = async (form: BookForm) => {
        const payload: BookRequestPayload = {
            ...
            // Send base64 url + file name; backend stores as-is
            images: form.images.map(img => ({ name: img.name, url: img.url })),
        }

        if (editBook) {
            await bookApi.updateBook(editBook.id, payload)
        ...
        <div className="bm">
            ...
            {toast && (
                <div style={{
                    ...
                    background: "var(--bg3)", border: "1px solid var(--border2,rgba(255,255,255,.12))",
            ...
            <div style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 28px" }}>
                ...
                <div className="bm-up" style={{ ...glass(), padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 16, animationDelay: "80ms" }}>
                    ...
                    {/* Category filter — use real categories */}
                    <select className="bm-input" value={filterCategory}
                                    ...
                                    return (
                                        <tr key={b.id} className="bm-row"
                                            ...
                                            <td style={{ padding: "13px 14px" }}>
                                                <span style={{ ...mono, fontSize: 10, padding: "3px 8px", borderRadius: 99, background: "rgba(255,255,255,.06)", color: "var(--muted2)" }}>
                                                    {b.category}
                                                </span>
                                            </td>
                                            ...
                                            <td style={{ padding: "13px 14px" }}>
                                                <span style={{ ...mono, fontSize: 10, fontWeight: 600, padding: "3px 9px", borderRadius: 99, background: st.bg, color: st.color, whiteSpace: "nowrap" }}>
                                                    {st.label}
                                                </span>
                                            </td>
                                            ...
                                            <td style={{ padding: "13px 14px", textAlign: "right" }}>
                                                <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                                                    <button className="bm-icon-btn" title="Edit"
                                                        ...
                                                        style={{ background: "rgba(255,255,255,.05)", border: "1px solid var(--border)", borderRadius: 7, width: 30, height: 30, fontSize: 13, cursor: "pointer", color: "var(--muted2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                        ✏️
                                                    </button>
                                                    <button className="bm-icon-btn" title="Delete"
                                                        onClick={() => setDeleteBook(b)}
                                                        style={{ background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", borderRadius: 7, width: 30, height: 30, fontSize: 13, cursor: "pointer", color: "var(--red)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                        🗑️
                                                    </button>
                                                </div>
                                            </td>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 4px", flexWrap: "wrap", gap: 12 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginRight: 10 }}>
                                ...
                                <select className="bm-input" value={pageSize}
