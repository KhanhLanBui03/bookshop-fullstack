                ...
                SELECT
                    ...
                    (SUM(oi.quantity) * 100.0 /
                        (
                            ...
                            FROM OrderItem oi2
                            WHERE oi2.order.orderStatus = com.fit.monolithic.backend.enums.OrderStatus.COMPLETED
                        )
                    ) as percent
                FROM OrderItem oi
                JOIN oi.book b
                JOIN b.category c
                WHERE oi.order.orderStatus = com.fit.monolithic.backend.enums.OrderStatus.COMPLETED
                GROUP BY c.name
